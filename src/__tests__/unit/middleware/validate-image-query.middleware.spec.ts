...
describe("validateImageQuery", () => {
  describe("missing width from query", () => {
    ...
    it("GET /api/images with missing width produces a helpful error", async () => {
      expect(response.text).toEqual(
        "Please append '&width=<WIDTH>' to the URL. Example: &width=500"
      );
    });
  ...
  describe("missing height from query", () => {
    ...
    it("GET /api/images with missing height produces 400 status code", () => {
      expect(response.status).toEqual(400);
    });
  
    it("GET /api/images with missing height produces a helpful error", () => {
      expect(response.text).toEqual(
